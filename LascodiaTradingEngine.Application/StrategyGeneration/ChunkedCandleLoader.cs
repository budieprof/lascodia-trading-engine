...
internal static class ChunkedCandleLoader
{
    ...
    internal static async Task LoadChunkedAsync(
        DbContext db,
        StrategyGenerationWorker.CandleLruCache candleCache,
        int screeningMonths,
        IReadOnlyList<string> symbols,
        Timeframe timeframe,
        int chunkSize,
        ...
        CancellationToken ct)
    {
        ...
        for (int i = 0; i < symbols.Count; i += chunkSize)
        {
            ...
            logger?.LogDebug(
                "ChunkedCandleLoader: loaded chunk {ChunkStart}-{ChunkEnd}, " +
                "{UncachedCount} uncached symbols, {CandleCount} candles fetched",
                i, i + chunk.Count - 1, uncached.Count, candles.Count);
        }
    }
}
