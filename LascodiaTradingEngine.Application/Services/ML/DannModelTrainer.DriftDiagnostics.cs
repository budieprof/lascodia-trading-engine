...
public sealed partial class DannModelTrainer
{
    ...
    /// </summary>
    private static DannDriftArtifact ComputeDannDriftDiagnostics(
        List<TrainingSample> trainSet, int F, string[] featureNames, double fracDiffD)
    {
        if (trainSet.Count < 30) return new DannDriftArtifact();
    ...
    private static void SanitizeDannSnapshotArrays(ModelSnapshot snapshot)
    {
        ...
        if (snapshot.DannWeights is not null)
            foreach (var row in snapshot.DannWeights) SanitizeDoubleArr(row);
    }
    ...
    private static double SafeDann(double v, double fallback = 0.0) => double.IsFinite(v) ? v : fallback;
    ...
    private static DannWarmStartArtifact BuildDannWarmStartArtifact(
        ModelSnapshot? warmStart, int featDim, int domHid, int F)
    {
        if (warmStart is null)
            return new DannWarmStartArtifact { Attempted = false, Compatible = false };

        var issues = new List<string>();
        int reusedLayers = 0;
        int totalParentLayers = 0;

        if (warmStart.DannWeights is { Length: > 0 } dw)
        {
            totalParentLayers = dw.Length;
            // Check layer 1 compatibility (each row should have F+1 elements)
            int parentFeatDim = 0;
            for (int r = 0; r < dw.Length; r++)
            {
                if (dw[r].Length != F + 1) { parentFeatDim = r; break; }
            }
            if (parentFeatDim == featDim)
            {
                reusedLayers += featDim; // layer 1 rows
                // Check layer 2 (should be featDim rows of featDim+1)
                if (parentFeatDim + featDim <= dw.Length)
                    reusedLayers += featDim; // layer 2 rows
                // Check classifier row
                if (2 * featDim < dw.Length)
                    reusedLayers++; // label classifier
            }
            else
            {
                issues.Add($"Parent featDim={parentFeatDim} != current featDim={featDim}");
            }
        }
        else
        {
            issues.Add("Parent snapshot has no DannWeights");
        }

        double reuseRatio = totalParentLayers > 0 ? (double)reusedLayers / totalParentLayers : 0.0;
        return new DannWarmStartArtifact
        {
            Attempted = true,
            Compatible = issues.Count == 0,
            ReusedLayerCount = reusedLayers,
            TotalParentLayers = totalParentLayers,
