...
public sealed partial class RocketModelTrainer
{
    ...
    /// </summary>
    private static RocketDriftArtifact ComputeRocketDriftDiagnostics(
        List<TrainingSample> trainSet, int F, string[] featureNames, double fracDiffD)
    {
        if (trainSet.Count < 30) return new RocketDriftArtifact();
        ...
        return new RocketDriftArtifact
        {
    ...
    private static void SanitizeRocketSnapshotArrays(ModelSnapshot snapshot)
    {
        ...
        SanitizeFloatArr(snapshot.FastFeatureAttribution);
        SanitizeDoubleArr(snapshot.JackknifeResiduals);
        SanitizeDoubleArr(snapshot.VennAbersCalBounds);
        SanitizeDoubleArr(snapshot.RocketFeatureMeans);
        SanitizeDoubleArr(snapshot.RocketFeatureStds);
        SanitizeDoubleArr(snapshot.RocketFeatureStats);
        SanitizeDoubleArr(snapshot.RocketMagWeights);
        SanitizeDoubleArr(snapshot.PerKernelAccuracyContribution);
        ...
        if (snapshot.RocketKernelWeights is not null)
            foreach (var row in snapshot.RocketKernelWeights) SanitizeDoubleArr(row);
    }
    ...
    private static double SafeRocket(double v, double fallback = 0.0) => double.IsFinite(v) ? v : fallback;
    ...
    private static double[] ComputeFeatureVariancesRocket(List<TrainingSample> trainSet, int F)
    {
    ...
    private static RocketWarmStartArtifact BuildRocketWarmStartArtifact(
        ModelSnapshot? warmStart, int numKernels, int retainedKernelCount)
    {
        if (warmStart is null || !string.Equals(warmStart.Type, "ROCKET", StringComparison.OrdinalIgnoreCase))
            return new RocketWarmStartArtifact { Attempted = false, Compatible = false };

        var issues = new List<string>();
        int totalParentKernels = warmStart.RocketKernelWeights?.Length ?? 0;

        if (totalParentKernels == 0)
            issues.Add("Parent snapshot has no RocketKernelWeights");
        ...
        double reuseRatio = totalParentKernels > 0 ? (double)retainedKernelCount / totalParentKernels : 0.0;
        return new RocketWarmStartArtifact
        {
            Attempted = true,
            Compatible = issues.Count == 0,
            ReusedKernelCount = retainedKernelCount,
            TotalParentKernels = totalParentKernels,
