...
public sealed partial class FtTransformerModelTrainer
{
    ...
    /// </summary>
    private static FtTransformerDriftArtifact ComputeFtTransformerDriftDiagnostics(
        List<TrainingSample> trainSet, int F, string[] featureNames, double fracDiffD)
    {
        if (trainSet.Count < 30) return new FtTransformerDriftArtifact();
        ...
        return new FtTransformerDriftArtifact
        {
    ...
    // ── Snapshot scalar sanitization ──────────────────────────────────────────

    private static void SanitizeFtTransformerSnapshotScalars(ModelSnapshot snapshot)
    {
        snapshot.Ece = SafeFt(snapshot.Ece, 1.0);
        snapshot.OptimalThreshold = SafeFt(snapshot.OptimalThreshold, 0.5);
        snapshot.BrierSkillScore = SafeFt(snapshot.BrierSkillScore, 0.0);
        snapshot.AvgKellyFraction = SafeFt(snapshot.AvgKellyFraction, 0.0);
        snapshot.TemperatureScale = SafeFt(snapshot.TemperatureScale, 0.0);
        snapshot.DurbinWatsonStatistic = SafeFt(snapshot.DurbinWatsonStatistic, 2.0);
        snapshot.PlattA = SafeFt(snapshot.PlattA, 1.0);
        snapshot.PlattB = SafeFt(snapshot.PlattB, 0.0);
        snapshot.FtTransformerCalibrationResidualMean = SafeFt(snapshot.FtTransformerCalibrationResidualMean, 0.0);
        snapshot.FtTransformerCalibrationResidualStd = SafeFt(snapshot.FtTransformerCalibrationResidualStd, 0.0);
        snapshot.FtTransformerCalibrationResidualThreshold = SafeFt(snapshot.FtTransformerCalibrationResidualThreshold, 1.0);
        snapshot.PredictionStabilityScore = SafeFt(snapshot.PredictionStabilityScore, 0.0);
        snapshot.CalibrationLoss = SafeFt(snapshot.CalibrationLoss, 0.0);
        snapshot.RefinementLoss = SafeFt(snapshot.RefinementLoss, 0.0);
    }

    private static double SafeFt(double v, double fallback = 0.0) => double.IsFinite(v) ? v : fallback;
