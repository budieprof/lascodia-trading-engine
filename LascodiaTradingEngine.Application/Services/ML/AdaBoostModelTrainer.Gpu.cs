...
public sealed partial class AdaBoostModelTrainer
{
        ...
        CancellationToken    ct = default)
    {
        ...
        try
        {
            return ComputeDensityRatioWeightsOnDevice(trainSet, F, recentWindowDays, barsPerDay, CUDA, ct);
        }
        catch
        {
            return null;
    ...
    /// <summary>
    /// GPU-accelerated adversarial validation. Moves the logistic classifier to CUDA.
    /// </summary>
        ...
        CancellationToken             ct = default)
    {
        ...
        try
        {
            return ComputeAdversarialAucOnDevice(trainSet, testSet, F, logger, CUDA, ct);
        }
        catch
        {
            return null;
    ...
    /// <summary>
    /// Device-generic density-ratio weight computation (CPU or CUDA).
    /// </summary>
    private static double[] ComputeDensityRatioWeightsOnDevice(
        List<TrainingSample> trainSet,
        int                  F,
        int                  recentWindowDays,
        int                  barsPerDay,
        ...
        CancellationToken    ct = default)
    {
        ...
        using var wP  = new TorchSharp.Modules.Parameter(zeros(F, 1, device: device));
        using var bP  = new TorchSharp.Modules.Parameter(zeros(1, device: device));
        using var opt = optim.Adam([wP, bP], lr: 0.01, weight_decay: 0.01);

        using var xTConst = tensor(xArr, device: device).reshape(n, F);
        using var yTConst = tensor(yArr, device: device).reshape(n, 1);

        for (int epoch = 0; epoch < 40; epoch++)
        {
            ct.ThrowIfCancellationRequested();
            opt.zero_grad();
            using var logit = mm(xTConst, wP) + bP;
            using var prob  = sigmoid(logit);
            using var loss  = nn.functional.binary_cross_entropy(prob, yTConst);
            loss.backward();
            opt.step();
        }
    ...
    /// <summary>
    /// Device-generic adversarial AUC computation (CPU or CUDA).
    /// </summary>
    private static double ComputeAdversarialAucOnDevice(
        List<TrainingSample>          trainSet,
        List<TrainingSample>          testSet,
        int                           F,
        ILogger<AdaBoostModelTrainer> logger,
    ...
    /// <summary>
    /// GPU-accelerated Platt scaling fit. Moves logistic regression to CUDA.
    /// </summary>
        ...
        CancellationToken    ct = default)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                double raw   = Math.Clamp(MLFeatureHelper.Sigmoid(2 * score), 1e-7, 1.0 - 1e-7);
