...
public sealed partial class GbmModelTrainer
{
    ...
    private static double[]? TryComputeDensityRatioWeightsGpu(
        List<TrainingSample> trainSet, int F, int recentWindowDays, int barsPerDay, int seed,
        ...
        CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            var xArr = new float[n * F];
            var yArr = new float[n];
            for (int i = 0; i < n0; i++) { Array.Copy(trainSlice[i].Features, 0, xArr, i * F, F); yArr[i] = 0f; }
