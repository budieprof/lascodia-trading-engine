...
{
    private readonly record struct BaggedLogisticAuditResult(
        string[] Findings,
        BaggedLogisticAuditArtifact Artifact);

    private static BaggedLogisticAuditResult RunBaggedLogisticAudit(
        ModelSnapshot snapshot,
        List<TrainingSample> auditSet)
    {
        var findings = new List<string>();
        var artifact = new BaggedLogisticAuditArtifact { SnapshotContractValid = true };
        ...
        if (auditSet.Count == 0)
        {
            ...
            return new BaggedLogisticAuditResult(artifact.Findings, artifact);
        }

        var engine = new EnsembleInferenceEngine();
        if (!engine.CanHandle(snapshot))
        {
            findings.Add("BaggedLogisticInferenceEngine refused the snapshot.");
            artifact.SnapshotContractValid = false;
            artifact.Findings = [.. findings];
            return new BaggedLogisticAuditResult(artifact.Findings, artifact);
        }
