...
internal static class WalkForwardRunClaimer
{
        ...
        CancellationToken ct)
    {
        ...
        var leaseExpiry = nowUtc.Add(WalkForwardExecutionLeasePolicy.LeaseDuration);
        var tableName = GetQuotedTableName(writeDb, typeof(WalkForwardRun));
        ...
        CancellationToken ct)
    {
        ...
        var expiredRuns = await writeDb.Set<WalkForwardRun>()
        ...
        {
            requeued = await writeDb.Set<WalkForwardRun>()
                ...
                .ExecuteUpdateAsync(setters => setters
                    ...
                    .SetProperty(run => run.AverageOutOfSampleScore, (decimal?)null)
                    .SetProperty(run => run.ScoreConsistency, (decimal?)null)
                    .SetProperty(run => run.WindowResultsJson, (string?)null), ct);
        }

        int orphaned = 0;
        if (toOrphan.Count > 0)
        {
            orphaned = await writeDb.Set<WalkForwardRun>()
                ...
                .ExecuteUpdateAsync(setters => setters
                    ...
                    .SetProperty(run => run.ErrorMessage, "Strategy deleted during walk-forward run")
                    ...
                    .SetProperty(run => run.FailureDetailsJson, ValidationRunException.SerializeDetails(new
                    {
                        ...
                        Reason = "Strategy deleted while walk-forward validation was executing."
                    }))
                    .SetProperty(run => run.ClaimedByWorkerId, (string?)null)
                    .SetProperty(run => run.ExecutionLeaseExpiresAt, (DateTime?)null)
    ...
    internal static void EnsureSupportedProvider(DbContext db)
    {
        ...
        throw new NotSupportedException(
            $"WalkForwardRunClaimer requires PostgreSQL ({PostgresProvider}) because it relies on FOR UPDATE SKIP LOCKED. Actual provider: {providerName ?? "<unknown>"}.");
    }
