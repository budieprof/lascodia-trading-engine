...
public class WalkForwardRunClaimerIntegrationTest : IClassFixture<PostgresFixture>
{
    private readonly PostgresFixture _fixture;

    public WalkForwardRunClaimerIntegrationTest(PostgresFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    public async Task ClaimNextRunAsync_ClaimsOldestEligibleQueuedRun_AndSkipsFutureQueuedRetry()
    {
        ...
        long eligibleStrategyId = await SeedStrategyAsync(seedCtx, "EligibleWalkForward", "EURUSD");
        long delayedStrategyId = await SeedStrategyAsync(seedCtx, "DelayedWalkForward", "GBPUSD");

        var nowUtc = new DateTime(2026, 04, 09, 10, 0, 0, DateTimeKind.Utc);
        var eligibleRun = new WalkForwardRun
        {
            StrategyId = eligibleStrategyId,
            Symbol = "EURUSD",
            Timeframe = Timeframe.H1,
            FromDate = nowUtc.AddDays(-60),
            ToDate = nowUtc.AddDays(-1),
            InSampleDays = 20,
            OutOfSampleDays = 10,
            InitialBalance = 10_000m,
            Status = RunStatus.Queued,
            StartedAt = nowUtc.AddHours(-2),
            QueuedAt = nowUtc.AddMinutes(-5),
            AvailableAt = nowUtc.AddMinutes(-5),
        };
        var delayedRun = new WalkForwardRun
        {
        ...
        seedCtx.Set<WalkForwardRun>().AddRange(eligibleRun, delayedRun);
        await seedCtx.SaveChangesAsync();

        await using var claimCtx = CreateWriteContext();
        var result = await WalkForwardRunClaimer.ClaimNextRunAsync(claimCtx, nowUtc, "itest-walkforward-claimer", CancellationToken.None);

        Assert.Equal(eligibleRun.Id, result.RunId);

        await using var verifyCtx = CreateReadContext();
        var claimedRun = await verifyCtx.Set<WalkForwardRun>().SingleAsync(r => r.Id == eligibleRun.Id);
        var untouchedRun = await verifyCtx.Set<WalkForwardRun>().SingleAsync(r => r.Id == delayedRun.Id);
    ...
    public async Task ClaimNextRunAsync_WhenCalledConcurrently_ClaimsDistinctRunsWithoutDuplicates()
    {
        ...
        await using var seedCtx = CreateWriteContext();
        long strategyAId = await SeedStrategyAsync(seedCtx, "WalkConcurrentA", "EURUSD");
        long strategyBId = await SeedStrategyAsync(seedCtx, "WalkConcurrentB", "GBPUSD");

        seedCtx.Set<WalkForwardRun>().AddRange(
            new WalkForwardRun
            {
                StrategyId = strategyAId,
                Symbol = "EURUSD",
                Timeframe = Timeframe.H1,
                FromDate = new DateTime(2026, 02, 01, 0, 0, 0, DateTimeKind.Utc),
        ...
        async Task<long?> ClaimAsync()
        {
            ...
            await using var claimCtx = CreateWriteContext();
            var result = await WalkForwardRunClaimer.ClaimNextRunAsync(
                claimCtx,
                new DateTime(2026, 04, 09, 10, 0, 0, DateTimeKind.Utc),
                "itest-walkforward-concurrent",
    ...
    public async Task RequeueExpiredRunsAsync_RequeuesActiveStrategyRun_AndClearsLeaseMetadata()
    {
        ...
        await using var seedCtx = CreateWriteContext();
        long strategyId = await SeedStrategyAsync(seedCtx, "WalkLeaseRecovery", "EURUSD");
        var nowUtc = new DateTime(2026, 04, 09, 10, 0, 0, DateTimeKind.Utc);
        var expiredRun = new WalkForwardRun
        {
        ...
        seedCtx.Set<WalkForwardRun>().Add(expiredRun);
        await seedCtx.SaveChangesAsync();

        await using var writeCtx = CreateWriteContext();
        var result = await WalkForwardRunClaimer.RequeueExpiredRunsAsync(writeCtx, nowUtc, CancellationToken.None);

        Assert.Equal((1, 0), result);

        await using var verifyCtx = CreateReadContext();
        var recoveredRun = await verifyCtx.Set<WalkForwardRun>().SingleAsync(r => r.Id == expiredRun.Id);
    ...
    public async Task RequeueExpiredRunsAsync_FailsDeletedStrategyRun_WithStructuredFailureCode()
    {
        ...
        {
            Name = "Deleted walk-forward strategy",
            Description = "deleted",
        ...
        var nowUtc = new DateTime(2026, 04, 09, 10, 0, 0, DateTimeKind.Utc);
        var orphanedRun = new WalkForwardRun
        {
        ...
        seedCtx.Set<WalkForwardRun>().Add(orphanedRun);
        await seedCtx.SaveChangesAsync();
        ...
        await using var verifyCtx = CreateReadContext();
        var failedRun = await verifyCtx.Set<WalkForwardRun>().SingleAsync(r => r.Id == orphanedRun.Id);
